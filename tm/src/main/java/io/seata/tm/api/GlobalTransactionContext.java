...
public class GlobalTransactionContext {
    ...
    private static GlobalTransaction getCurrent() {
        ...
        return new DefaultGlobalTransaction(xid, GlobalStatus.Begin, GlobalTransactionRole.Participant); //创建默认的全局事务对象
    }
    ...
    public static GlobalTransaction getCurrentOrCreate() {
        GlobalTransaction tx = getCurrent(); //获取当前的全局事务
        if (tx == null) {
            return createNew(); //如果没有则进行创建
        }
        return tx;
    }
