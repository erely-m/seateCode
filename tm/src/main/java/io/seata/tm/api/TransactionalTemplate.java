...
public class TransactionalTemplate {
     ...
     */
    public Object execute(TransactionalExecutor business) throws Throwable { //实际开启
        ...
        try {
            ...
            // 2. begin transaction
            beginTransaction(txInfo, tx); //请求TC记录全局事务开始

            Object rs = null;
            try {

                // Do Your Business
                rs = business.execute(); //执行业务

            } catch (Throwable ex) { //如果业务存在异常则需要进行回滚
            ...
            commitTransaction(tx); // 一切ok 提交事务
    ...
    private void completeTransactionAfterThrowing(TransactionInfo txInfo, GlobalTransaction tx, Throwable ex) throws TransactionalExecutor.ExecutionException {
        //roll back
        if (txInfo != null && txInfo.rollbackOn(ex)) { //判断是否需要回滚
            try {
                rollbackTransaction(tx, ex); //回滚事务
    ...
    private void rollbackTransaction(GlobalTransaction tx, Throwable ex) throws TransactionException, TransactionalExecutor.ExecutionException {
        ...
        throw new TransactionalExecutor.ExecutionException(tx, TransactionalExecutor.Code.RollbackDone, ex); //回滚完毕抛出异常
    }

    private void beginTransaction(TransactionInfo txInfo, GlobalTransaction tx) throws TransactionalExecutor.ExecutionException {
        try {
            triggerBeforeBegin(); //开始前做一些处理
