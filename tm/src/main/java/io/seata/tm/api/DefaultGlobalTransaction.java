...
public class DefaultGlobalTransaction implements GlobalTransaction {
    ...
    @Override
    public void begin(int timeout, String name) throws TransactionException { //开始全局事务
    ...
    public void commit() throws TransactionException {
        ...
        try {
            while (retry > 0) {
                try {
                    status = transactionManager.commit(xid); //提交全局事务
                    break;
                } catch (Throwable ex) {
    ...
    public void rollback() throws TransactionException {
        ...
        try {
            while (retry > 0) {
                try {
                    status = transactionManager.rollback(xid); //发送回滚消息给TC  TC通知分支进行回滚
                    break;
                } catch (Throwable ex) {
