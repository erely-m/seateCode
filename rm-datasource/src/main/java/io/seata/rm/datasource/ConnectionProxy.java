 ...
 * @author sharajava
 */
public class ConnectionProxy extends AbstractConnectionProxy { //连接代理类
    ...
    private void doCommit() throws SQLException {
        if (context.inGlobalTransaction()) { //全局事务
            processGlobalTransactionCommit();
        } else if (context.isGlobalLockRequire()) { //全局锁？ TODO
            processLocalCommitWithGlobalLocks();
        } else {
            targetConnection.commit(); //正常提交
        }
    }
    ...
    private void processGlobalTransactionCommit() throws SQLException {
        ...
        try {
            if (context.hasUndoLog()) {
                UndoLogManagerFactory.getUndoLogManager(this.getDbType()).flushUndoLogs(this); //插入undo信息至undoLog表中
            }
            targetConnection.commit(); //进行事务提交
        } catch (Throwable ex) {
        ...
        if (IS_REPORT_SUCCESS_ENABLE) {
            report(true); //报告分支状态 至TC
        }
        context.reset(); //释放ConnectionContext
    }

    private void register() throws TransactionException { //注册
        Long branchId = DefaultResourceManager.get().branchRegister(BranchType.AT, getDataSourceProxy().getResourceId(),
            null, context.getXid(), null, context.buildLockKeys()); //注册分支
        context.setBranchId(branchId); //设置分支ID
    }
