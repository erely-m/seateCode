...
public abstract class AbstractDMLBaseExecutor<T, S extends Statement> extends BaseTransactionalExecutor<T, S> {
    ...
    protected T executeAutoCommitFalse(Object[] args) throws Exception {
        TableRecords beforeImage = beforeImage(); //执行前镜像复制
        T result = statementCallback.execute(statementProxy.getTargetStatement(), args); //执行具体语句
        TableRecords afterImage = afterImage(beforeImage); //执行后的镜像复制
        prepareUndoLog(beforeImage, afterImage); //准备undoLog日志
        return result;
    }
    ...
    protected T executeAutoCommitTrue(Object[] args) throws Throwable {
        AbstractConnectionProxy connectionProxy = statementProxy.getConnectionProxy();
        try {
            connectionProxy.setAutoCommit(false); //自动提交设置为false
            ...
            connectionProxy.setAutoCommit(true); //恢复自动提交
        }
    }
