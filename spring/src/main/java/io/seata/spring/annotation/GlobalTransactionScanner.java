...
public class GlobalTransactionScanner extends AbstractAutoProxyCreator
    ...
    private void initClient() {
        ...
        //init TM
        TMClient.init(applicationId, txServiceGroup); //初始化 TM
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(
    ...
    @Override
    protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) { //增强
    ...
    @Override
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException { //对DataSource层进行增强
        if (bean instanceof DataSource && !(bean instanceof DataSourceProxy) && ConfigurationFactory.getInstance().getBoolean(DATASOURCE_AUTOPROXY, false)) { //判断bean的类型是不是DataSource 如果是进行处理
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("Auto proxy of [{}]", beanName);
            }
            DataSourceProxy dataSourceProxy = DataSourceProxyHolder.get().putDataSource((DataSource) bean); //缓存 并进行创建DataSourceProxy
            Class<?>[] interfaces = SpringProxyUtils.getAllInterfaces(bean); //获取所有的接口类
            return Proxy.newProxyInstance(Thread.currentThread().getContextClassLoader(), interfaces, new InvocationHandler() { //JDK进行动态增强
                @Override
                public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                    Method m = BeanUtils.findDeclaredMethod(DataSourceProxy.class, method.getName(), method.getParameterTypes()); //寻找默认方法
                    if (null != m) {
                        return m.invoke(dataSourceProxy, args); //使用增强后的bean去进行调用
                    } else {
                        boolean oldAccessible = method.isAccessible();
                        try {
                            method.setAccessible(true);
                            return method.invoke(bean, args); //使用原始的bean进行调用
                        } finally {
                            //recover the original accessible for security reason
                            method.setAccessible(oldAccessible); //恢复原始访问权限
                        }
                    }
                }
            });
